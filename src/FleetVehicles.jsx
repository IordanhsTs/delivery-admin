          ...
          {rows.map((v, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Δύο ΔΙΑΦΟΡΕΤΙΚΑ πράγματα, γι' αυτό και δύο γραμμές: το πρώτο
                    φωνάζει («ζημιά που δεν τακτοποιήθηκε»), το δεύτερο απλώς
                    λέει ότι η μηχανή έχει ιστορικό. */}
                {Number(v.open_notes_count) > 0 ? (
                  <div className="mt-3">
                    <Pill tone="danger" icon={AlertTriangle}
                      title="Ζημιές ή επισκευές που δεν έχουν σημανθεί ως διευθετημένες">
                      {plural(v.open_notes_count, 'εκκρεμεί', 'εκκρεμούν')}
                    </Pill>
                  </div>
                ) : null}

                {Number(v.notes_count) > 0 ? (
                  <div className="mt-2 text-[11px] flex items-center gap-1.5" style={{ color: 'var(--text-muted)' }}>
...
function NotesSection({ vehicle, notes, loading, onSaved }) {
  ...
  const [saving, setSaving] = useState(false);
  // Ποια γραμμή περιμένει απάντηση από τη βάση — για να μη διπλοπατηθεί το τικ
  // και να φαίνεται πού «τρέχει» κάτι.
  const [busyId, setBusyId] = useState(null);
  // Οι διευθετημένες μαζεύονται πίσω από διακόπτη: ο πελάτης ανοίγει την καρτέλα
  // για να δει ΤΙ ΕΚΚΡΕΜΕΙ, όχι το αρχείο δύο ετών. Το ιστορικό μένει ένα κλικ
  // μακριά — δεν κρύβεται, απλώς δεν είναι το πρώτο πράγμα που διαβάζεις.
  const [showResolved, setShowResolved] = useState(false);

  const openNotes     = notes.filter((n) => !n.resolved_at);
  const resolvedNotes = notes.filter((n) => n.resolved_at);
  const visibleNotes  = showResolved ? notes : openNotes;

  /** Τικ «διευθετήθηκε» / αναίρεσή του. */
  async function toggleResolved(note) {
    setBusyId(note.id);
    const { data: sess } = await supabase.auth.getSession();
    const { error } = await supabase.from('vehicle_notes').update(
      note.resolved_at
        ? { resolved_at: null, resolved_by: null }
        : { resolved_at: new Date().toISOString(), resolved_by: sess?.session?.user?.id ?? null },
    ).eq('id', note.id);
    setBusyId(null);
    if (error) { toast.error('Δεν ενημερώθηκε: ' + error.message); return; }
    toast.success(note.resolved_at ? 'Επανήλθε σε εκκρεμές.' : 'Διευθετήθηκε.');
    onSaved();
  }

  /** Οριστική διαγραφή — δεν υπάρχει «κάδος» εδώ, γι' αυτό ρωτάει. */
  async function removeNote(note) {
    const ok = await confirmDialog(
      'Η σημείωση θα διαγραφεί οριστικά και δεν επαναφέρεται. Αν απλώς τακτοποιήθηκε,'
      + ' προτίμησε το τικ διευθέτησης — έτσι μένει στο ιστορικό της μηχανής.',
      { title: 'Διαγραφή σημείωσης', confirmLabel: 'Διαγραφή', danger: true },
    );
    if (!ok) return;
    setBusyId(note.id);
  ...
  return (
    <section className="p-4 space-y-4 card-surface" style={cardStyle}>
      ...
      {/* Διακόπτης ιστορικού — μόνο όταν υπάρχει ιστορικό να δείξει. */}
      {resolvedNotes.length > 0 ? (
        <button onClick={() => setShowResolved((s) => !s)}
          ...
          <Archive size={13} />
          {showResolved
            ? 'Απόκρυψη διευθετημένων'
            : `Εμφάνιση ${plural(resolvedNotes.length, 'διευθετημένης', 'διευθετημένων')}`}
        </button>
      ) : null}
      ...
      {loading ? (
        ...
        <p className="text-sm text-center py-4" style={{ color: 'var(--text-muted)' }}>
          Δεν εκκρεμεί τίποτα — όλες οι σημειώσεις έχουν διευθετηθεί.
        </p>
      ) : (
        <ul className="space-y-2 list-none p-0 m-0">
          {visibleNotes.map((n) => {
            const meta = KINDS.find((k) => k.value === n.kind) || KINDS[0];
            const done = !!n.resolved_at;
            return (
              <li key={n.id} className="p-3 rounded-lg"
                // Η διευθετημένη ξεθωριάζει αντί να φύγει: παραμένει αναγνώσιμη
                // ως ιστορικό, αλλά δεν τραβάει το μάτι πάνω από ό,τι εκκρεμεί.
                style={{ border: '1px solid var(--border-subtle)', opacity: done ? 0.6 : 1 }}>
                <div className="flex items-center justify-between gap-2 mb-1.5">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <Pill tone={done ? 'muted' : n.kind === 'damage' ? 'danger' : n.kind === 'service' ? 'success' : 'muted'} icon={meta.icon}>
                      {meta.label}
                    </Pill>
                    {done ? (
                      <Pill tone="success" icon={Check} title={`Διευθετήθηκε ${fmtWhen(n.resolved_at)}`}>
                        Διευθετήθηκε
                      </Pill>
                    ) : null}
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-3 mt-2 text-[11px]" style={{ color: 'var(--text-muted)' }}>
                  ...
                  {/* Οι ενέργειες σπρώχνονται δεξιά — δεν μπερδεύονται με τα στοιχεία. */}
                  <span className="ml-auto flex items-center gap-1.5">
                    <button onClick={() => toggleResolved(n)} disabled={busyId === n.id}
                      title={done ? 'Επαναφορά σε εκκρεμές' : 'Σήμανση ως διευθετημένη'}
                      className="px-2 py-1 rounded-md text-[11px] font-bold flex items-center gap-1 disabled:opacity-50"
                      style={done
                      ...
                      {done ? <><Undo2 size={11} /> Αναίρεση</> : <><Check size={11} /> Διευθετήθηκε</>}
                    </button>
                    <button onClick={() => removeNote(n)} disabled={busyId === n.id}
                      title="Οριστική διαγραφή σημείωσης"
